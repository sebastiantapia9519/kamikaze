...
const PlayerTurnIndicator = ({ currentPlayer, players = [], challenge }) => {
    ...
    return (
        <div className="space-y-4">
            {/* Current Turn */}
            <div className={`bg-gradient-to-r ${getCategoryColor()} p-4 rounded-xl shadow-graffiti-md text-white text-center relative overflow-hidden`}>
                {/* Background pattern */}
                ...
                {players?.map((player, index) => (
                    <div
                        ...
                        className={`px-3 py-2 rounded-lg border transition-all duration-200 ${player?.id === currentPlayer?.id
                                ? 'bg-primary text-primary-foreground border-primary shadow-graffiti-sm'
